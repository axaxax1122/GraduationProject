...
protected:
	bool CollisionSphereToSphere(const Vec3& vC1, float fR1, const Vec3& vC2,
		float fR2);
	bool CollisionOBB(const OBBINFO &tInfo1, const OBBINFO &tInfo2);

	bool CollisionRectToRect(const RECTINFO& tInfo1, const RECTINFO& tInfo2);
	bool CollisionRectToPoint(const RECTINFO& tRC, const Vec2& vPos);
};
