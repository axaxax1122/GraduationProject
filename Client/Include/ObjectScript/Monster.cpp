...
int CMonster::Update(float fTime)
{
	...
	else
	{
		...
		CDx11ColliderOBB*	pOBB = (CDx11ColliderOBB*)pPlayerObj->FindComponentFromTypeName("CDx11ColliderOBB");
		Vec3	vCenter = pOBB->GetCenter();
		SAFE_RELEASE(pOBB);

		// m_vDir을 구한다.
		Vec3 vDir = vCenter - m_pTransform->GetWorldPos();
		...
		if (m_pTransform->GetWorldPos().Distance(vCenter) > 50.f)
		{
			m_pTransform->MoveWorld(m_fSpeed, fTime * 4, m_pTransform->GetWorldAxis(AXIS_Y));
		}
		else
		{
...
CDx11GameObject * CMonster::CreateBullet(CDx11Transform * pTransform, const Vec3 & vDir)
{
	...
	pBulletTr->MoveWorld(2.f, 1.f, pBulletTr->GetWorldAxis(AXIS_Z));
	pBulletTr->SetWorldScale(0.15f, 0.2f, 0.15f);
