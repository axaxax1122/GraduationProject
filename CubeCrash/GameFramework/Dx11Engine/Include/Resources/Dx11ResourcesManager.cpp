...
bool CDx11ResourcesManager::Init()
{
	...
	UINT	iUIIndex2[6] = { 0, 1, 3, 0, 3, 2 };

	pRectangleMesh = CreateMesh("RectCenterUIMesh", sizeof(UIVERTEX),
		4, tUIVtx2, sizeof(UINT), 6, iUIIndex2,
		DXGI_FORMAT_R32_UINT);
	SAFE_RELEASE(pRectangleMesh);
	...
	pSampler = CreateSampler(POINT_SAMPLER, D3D11_FILTER_MIN_MAG_MIP_POINT);
	SAFE_RELEASE(pSampler);
...
CDx11Mesh * CDx11ResourcesManager::LoadMesh(const string & strKey, 
	wchar_t * pFileName, const string & strPathKey)
{
	CDx11Mesh*	pMesh = FindMesh(strKey);
...
CDx11Mesh * CDx11ResourcesManager::LoadMesh(const string & strKey, wchar_t * pFileName, vector<vector<wchar_t*>>& vecTexName, const string & strPathKey)
{
	CDx11Mesh*	pMesh = FindMesh(strKey);
...
CDx11Texture * CDx11ResourcesManager::LoadTexture(const string & strKey, wchar_t * pFileName, const string & strPathKey)
{
...
CDx11Texture * CDx11ResourcesManager::CreateTexture(const string & strKey, UINT iWidth, UINT iHeight, UINT iArraySize, DXGI_FORMAT eFmt, D3D11_USAGE eUsage, D3D11_BIND_FLAG eBindFlag, int iCpuFlag)
{
	CDx11Texture*	pTexture = FindTexture(strKey);
