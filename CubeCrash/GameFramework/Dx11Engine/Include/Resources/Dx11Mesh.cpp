...
bool CDx11Mesh::LoadMesh(const string& strKey, wchar_t * pFileName, 
...
bool CDx11Mesh::LoadMesh(const string & strKey, wchar_t * pFileName, 
	vector<vector<wchar_t*>>& vecTexName, const string & strPathKey)
{
	m_eTopology = D3D_PRIMITIVE_TOPOLOGY_TRIANGLELIST;
	...
	if (iPathPos == -1)
	{
		memcpy(strFileName, pFileName, sizeof(wchar_t) * (iDotPos));
	}
	// 매쉬가 폴더에 속한 경우
	else
	{
		...
		memcpy(strFileName, &pFileName[iPathPos + 1], sizeof(wchar_t) * iLength);
	}
...
bool CDx11Mesh::LoadVertexData(const string & strPathKey)
{
	...
	for (size_t i = 0; i < m_iContainerCount; ++i)
	{
		for (int j = 0; j < vecMeshContainer[i]->vecMaterial.size(); ++j)
		{
			...
			if (!vecMeshContainer[i]->vecMaterial[j]->strDiffuseTex.empty())
			{
				...
				pMaterial->SetPointSampler(POINT_SAMPLER);
			}
...
bool CDx11Mesh::LoadMaterialData(const string & strKey, 
	wchar_t * pTexName, const string & strPathKey)
{
	vector<PMESHDATA>&	vecMeshContainer = m_pLoader->GetMeshContainer();
...
bool CDx11Mesh::LoadAnimationData(const string & strPathKey)
{
	m_pAnimation = new CDx11Animation3D;
	if (!m_pAnimation->Init())
		return false;

	m_pAnimation->SetMeshName(strPathKey);

	// Animation Clip 정보를 생성한다.
	vector<PFBXANIMATIONCLIP>& vecClip = m_pLoader->GetAnimationClip();
	for (size_t i = 0; i < vecClip.size(); ++i)
	{
		...
		m_pAnimation->AddAnimationClip(tClip);
	}

	// 로더에서 본 정보를 얻어온다.
	vector<FBXBONE>&	vecBones = m_pLoader->GetBones();
