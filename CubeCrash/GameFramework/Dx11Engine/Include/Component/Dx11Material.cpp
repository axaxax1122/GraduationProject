...
void CDx11Material::SetInfo(const Vec3 & vDif, const Vec3 & vAmb, 
	const Vec3 & vSpc, const Vec3 & vEmv, float fPower, float fFadeAmount)
{
	...
	m_tInfo.fFadeAmount = fFadeAmount;
}
...
bool CDx11Material::SetPointSampler(const string & strKey)
{
	SAFE_RELEASE(m_pPointSampler);

	m_pPointSampler = DX11_GET_SINGLE(CDx11ResourcesManager)->FindSampler(strKey);
	if (!m_pPointSampler)
		return false;
	...
	UINT iRegisterTex, UINT iRegisterSampler)
{
	...
	if (m_pDiffuseSampler)
		m_pDiffuseSampler->SetSampler(0);

	if (m_pPointSampler)
		m_pPointSampler->SetSampler(1);
}
