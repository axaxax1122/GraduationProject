...
bool CDx11ShaderManager::Init()
{
	...
	// 애니메이션 쉐이더를 로드한다.
	pEntry[ST_VERTEX] = "DefaultAnimationVS";
	pEntry[ST_PIXEL] = "DefaultBumpPS";
	pShader = LoadShader(ANIMATION_SHADER, L"Default.fx", pEntry, 5, 0);

	AddInputElement(ANIMATION_SHADER, "POSITION", 0, DXGI_FORMAT_R32G32B32_FLOAT,
		0, 12, D3D11_INPUT_PER_VERTEX_DATA, 0);
	AddInputElement(ANIMATION_SHADER, "NORMAL", 0, DXGI_FORMAT_R32G32B32_FLOAT,
		0, 12, D3D11_INPUT_PER_VERTEX_DATA, 0);
	AddInputElement(ANIMATION_SHADER, "TEXCOORD", 0, DXGI_FORMAT_R32G32_FLOAT,
		0, 8, D3D11_INPUT_PER_VERTEX_DATA, 0);
	AddInputElement(ANIMATION_SHADER, "TANGENT", 0, DXGI_FORMAT_R32G32B32_FLOAT,
		0, 12, D3D11_INPUT_PER_VERTEX_DATA, 0);
	AddInputElement(ANIMATION_SHADER, "BINORMAL", 0, DXGI_FORMAT_R32G32B32_FLOAT,
		0, 12, D3D11_INPUT_PER_VERTEX_DATA, 0);
	AddInputElement(ANIMATION_SHADER, "WEIGHTS", 0, DXGI_FORMAT_R32G32B32A32_FLOAT,
		0, 16, D3D11_INPUT_PER_VERTEX_DATA, 0);
	AddInputElement(ANIMATION_SHADER, "BLENDINDICES", 0, DXGI_FORMAT_R32G32B32A32_FLOAT,
		0, 16, D3D11_INPUT_PER_VERTEX_DATA, 0);
	...
	pShader->CreateConstantBuffer("LightPoint", sizeof(Vec4), 4);
	pShader->CreateConstantBuffer("FogInfo", sizeof(FOGINFO), 5);
	pShader->CreateConstantBuffer("BoneArray", sizeof(BONECBUFFER), 6);

	CreateInputLayout(ANIMATION_SHADER);
	SAFE_RELEASE(pShader);
	...
	pShader->CreateConstantBuffer("Transform", sizeof(TRANSFORMCBUFFER), 0);
	pShader->CreateConstantBuffer("UVFrame", sizeof(UVFRAME), 7);
	...
	pShader->CreateConstantBuffer("Transform", sizeof(TRANSFORMCBUFFER), 0);
	pShader->CreateConstantBuffer("Color", sizeof(Vec4), 7);
...
CDx11Shader * CDx11ShaderManager::LoadShader(const string & strKey,
	wchar_t * pFileName, char * pEntry[ST_MAX], int iHighV, int iLowV,
