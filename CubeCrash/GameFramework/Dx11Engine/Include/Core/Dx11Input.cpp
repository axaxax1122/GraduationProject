...
bool CDx11Input::GetActivated() const
{
	return m_bActivated;
}

bool CDx11Input::GetControl() const
{
	return m_bControl;
}

HWND CDx11Input::GetHWND() const
{
	return m_hWnd;
}
...
bool CDx11Input::Init(HWND hWnd)
{
	...
	//CDx11ColliderRay* pRayColl = m_pMouseObj->AddComponent<CDx11ColliderRay>("RayCollider");
	//SAFE_RELEASE(pRayColl);
	...
	CreateKey("Roll", VK_CONTROL);
	CreateKey("Lying", VK_SHIFT);
	CreateKey('1', "Skill1");
...
int CDx11Input::Update(float fTime)
{
	...
	pQuadTree->AddDynamicObj(m_pMouseObj);
...
void CDx11Input::SetActivated(bool bActivated)
{
	...
	if (m_bActivated)
		if (m_bControl)
			if (m_pMouse)
				m_pMouse->SetMousePositionToCenter();
}

void CDx11Input::SetControl(bool bControl)
{
	m_bControl = bControl;
}
