...
bool CDx11Core::Init(wchar_t * pTitle, wchar_t * pClass,
...
bool CDx11Core::Init(HWND hWnd, RESOLUTION_TYPE eRT, bool bWindowMode)
{
	// 디버그를 초기화한다.
	...
	// 디바이스를 초기화한다.
	if (!DX11_GET_SINGLE(CDx11Device)->Init(eRT, bWindowMode, hWnd))
		return false;
...
void CDx11Core::LogicUpdate(float fFrameTime)
{
	if (Update(fFrameTime) == SC_CHANGE)
		return;
...
void CDx11Core::Collision(float fTime)
{
	...
	DX11_GET_SINGLE(CDx11SceneManager)->Collision(pQuadTree, fTime);
