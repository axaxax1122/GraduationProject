...
void CDx11FbxLoader::LoadAnimationData(FbxMesh * pMesh, PMESHDATA pMeshData)
{
	...
	if (bDeformer)
	{
		// AnimationClip 정보를 로드한다.
		if (!LoadAnimationClipData(pMesh, pMeshData))
			return;
...
bool CDx11FbxLoader::LoadAnimationClipData(FbxMesh * pMesh, PMESHDATA pMeshData)
{
	// 애니메이션 이름을 이용해여 스택을 얻어온다.
	FbxAnimStack*	pAnimStack = m_pScene->FindMember<FbxAnimStack>(m_AnimNameArray[m_iMeshCount]->Buffer());
	if (!pAnimStack)
		return false;
	...
	const FbxAMatrix & matGeometryTransform, int iBoneIndex, PMESHDATA pMeshData)
{
	...
	FbxLongLong lStart = pMeshData->pAnimationClip->startTime.GetFrameCount(FbxTime::eFrames30);
	FbxLongLong lEnd = pMeshData->pAnimationClip->endTime.GetFrameCount(FbxTime::eFrames30);
...
void CDx11FbxLoader::ChangeVertexWeightAndIndices(FbxMesh * pMesh, PMESHDATA pMeshData)
{
	...
	{
		Vec3	vPos;
		vPos.x = pControlPoints[iter->first][0];
		vPos.y = pControlPoints[iter->first][2];
		vPos.z = pControlPoints[iter->first][1];
