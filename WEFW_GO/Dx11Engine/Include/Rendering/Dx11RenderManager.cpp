...
bool CDx11RenderManager::IsInFrustrum(CDx11GameObject * pObj)
{
	...
	for (UINT i = 0; i < 6; ++i)
	{
		...
		// d > 0 이면 평면 앞 (바깥쪽)
		// d < 0 이면 평면 뒤 (안쪽)
		XMVECTOR v = XMPlaneDotCoord(fs.m_vPlane[i], pTr->GetWorldPos().Convert());
		...
		if (pColl)
		{
			...
			// 구의 반지름보다 크면 바깥쪽에 있는 것임.
			if (v.vector4_f32[0] > fRadius * 2)
			{
				SAFE_RELEASE(pTr);
				SAFE_RELEASE(pCam);
